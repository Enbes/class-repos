...
let rec mytreeMap (f : 'a -> 'b) (t : 'a mytree) : 'b mytree =
  match t with
  | Leaf -> Leaf
  | Node (v, l, r) -> Node (f v, mytreeMap f l, mytreeMap f r)
;;

Printf.printf "EXERCISE 11\n";;

if mytreeMap odd (Node (1, Leaf, Leaf)) = Node (true, Leaf, Leaf)
then Printf.printf ""
else Printf.printf "T1 fail\n"
;;
  
if mytreeMap even (Node (1, Node (2, Leaf, Leaf), Leaf)) = Node (false, Node (true, Leaf, Leaf), Leaf)
then Printf.printf ""
else Printf.printf "T2 fail\n"
;;
  
if mytreeMap odd (Node (1, Leaf, Node (3, Leaf, Leaf))) = Node (true, Leaf, Node (true, Leaf, Leaf))
then Printf.printf ""
else Printf.printf "T3 fail\n"
;;
  
if mytreeMap odd (Node (1, Node (2, Leaf, Leaf), Node (3, Leaf, Leaf))) = (Node (true, Node (false, Leaf, Leaf), Node (true, Leaf, Leaf)))
then Printf.printf ""
else Printf.printf "T4 fail\n"
;;
  
if mytreeMap decToBin (Node (1, Node (2, Leaf, Leaf), Leaf)) = Node ([true], Node ([false; true], Leaf, Leaf), Leaf)
then Printf.printf ""
else Printf.printf "T5 fail\n"
;;
